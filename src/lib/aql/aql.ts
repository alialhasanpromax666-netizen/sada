...
function bina2TaalimatNizam(talab: TalabTawlid): string {
  ...
  const sutur = [
    ...
    `مهمّتك: ${wakeel.wasf}`,
    ``,
    `⏰ التاريخ الحالي: ${new Date().toLocaleDateString("ar-SA")} (عام 2026).`,
    `لا تستخدم معلومات قديمة. أي إشارة لعام أو معلومة يجب أن تكون محدّثة حتى 2026.`,
    ``,
    `صوتك وشخصيّتك:`,
    `- النبرة: ${sh.nabra}`,
    `- الأسلوب: ${sh.uslub}`,
  ...
  sutur.push(
    ``,
    `نحن في عام 2026. أي معلومات تذكرها يجب أن تكون محدّثة حتى 2026. لا تستخدم معلومات قديمة.`,
    ``,
